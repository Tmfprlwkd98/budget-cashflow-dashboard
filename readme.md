...
- **무서버**: 모든 데이터는 내 브라우저(localStorage)에만 저장
...
2. `index.html`의 `<head>`에서 스크립트를 아래로 교체합니다:
   ...
   ```
> 선택: VS Code **Live Server/Live Preview** 로 띄우면 자동 새로고침/경로 확인이 편합니다만, 필수는 아닙니다.
...
,2025-11-01,스타벅스,카페,expense,5600
,2025-11-02,지하철,교통,expense,1450
```
...
## 기타 참고사항
- **CSV 인코딩**은 UTF-8로 저장하세요(엑셀: “CSV UTF-8(쉼표로 분리)” 권장).
- **시크릿(인프라이빗) 모드**에서는 localStorage가 지워질 수 있습니다.
- 일부 환경에서 **외부 CDN**이 차단되면 차트가 보이지 않습니다(로컬 차트 파일 권장).